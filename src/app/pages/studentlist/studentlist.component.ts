...
export class StudentlistComponent implements OnInit {
  ...
  // ================= EXTEND =================
  openExtendModal() {
    if (!this.selectedStudent || !this.selectedStudent.subscription) return;
  ...
  deleteUser() {
    ...
    this.studentservice.deleteStudent(this.selectedStudent._id).subscribe({
      next: () => {
  ...
  blockUser() {
    ...
    this.studentservice.blockStudent(this.selectedStudent._id).subscribe({
      next: () => {
  ...
  UnblockUser() {
    ...
    this.studentservice.unblockStudent(this.selectedStudent._id).subscribe({
      next: () => {
  ...
  DelinkUser() {
    if (!this.selectedStudent) return;
    if (!confirm('Want to remove Device Id of this User?')) return;

    this.studentservice.delinkStudent(this.selectedStudent._id).subscribe({
      next: () => {
        alert('User device has been removed successfully');
        this.closeAllModals();
        this.loadStudents();
      },
      error: (err) => {
